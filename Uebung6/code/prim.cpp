...
std::ostream & operator<<(std::ostream & os, const std::vector<Edge> & E) 
{
    ...
    for (size_t i = 0, N = E.size(); i < N; ++i)
        os << (i > 0 ? "," : "") << "(" << static_cast<char>('a' + E[i].vi2) << "," << static_cast<char>('a' + E[i].vi1) << " - " << E[i].weight << ")";
    os << "}";
    return os;
}
...
void createGraph(std::vector<Edge> & E, std::vector<Vertex> & V) 
{
	...
	for (int i=0; i<N; i++) {
		...
		//insert edges
		for (int j=0; j<i; j++) {	//prevent making loops and duplicates with condition j<i
			if (weight(i,j) != Edge::NO_EDGE) {
...
{   
    ...
    int W = 0;
	std::for_each(E.begin(), E.end(), [&W](const Edge &e){W+=e.weight;});
...
bool operator<(const Vertex &v1, const Vertex &v2)
{
	...
	if (v2.deleted) {return true;}
	if (v1.deleted) {return false;}
	return (v1.key < v2.key);
}

void updateNeighbors(const Vertex & selected, std::vector<Vertex> & V)
{
	for (auto & neighbor : V) {
		if (weight(selected.index, neighbor.index) == Edge::NO_EDGE) {
			continue;
		}
		if (neighbor.deleted) {
			continue;
		}
		if  (neighbor.key > (weight(selected.index, neighbor.index))) {
			neighbor.key = weight(selected.index, neighbor.index);
			neighbor.parent_index = selected.index;
		}
	}
}
...
void prim() 
{
	...
	createGraph(E, V);
	std::cout << "Created graph with\nVertices: " << V << "\nEdges: " << E << "\nTotalweight Edges: " << totalWeight(E) << std::endl;
	...
	bool VerticesRemaining = true;
	while (VerticesRemaining) {
		updateNeighbors(V[0], V);

		V[0].deleted = true;
		std::sort(V.begin(), V.end());

		if (V[0].deleted) {		//deleted attribute is also used in sort
			VerticesRemaining = false;
		} else {
			int selectedI = V[0].index;
			int parentI = V[0].parent_index;
			Edge e = (selectedI > parentI) ? Edge(selectedI, parentI) : Edge(parentI, selectedI);
			mst.push_back(e);
		}
	}

	std::cout << "Created minimal spanning Tree with\nVertices: " << V << "\nEdges: " << mst << "\nTotalweight Edges: " << totalWeight(mst) << std::endl;
}
